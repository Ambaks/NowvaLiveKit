/* Captures the display UI during the condensed replay into out/ui.webm.
...
const py = spawn(path.join(ROOT, "venv", "bin", "python"),
  [path.join(HERE, "replay_condensed.py")], { cwd: ROOT });
...
const recorder = await page.screencast({ path: path.join(OUT, "ui.webm") });
await waitLine("DONE");
await recorder.stop();
await browser.close();
py.kill("SIGINT");
console.log(path.join(OUT, "ui.webm"));
