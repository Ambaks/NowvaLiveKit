#!/bin/bash
# Assembles the SILENT promo:
#   1. render the 3D rack shot with out/ui_tex.mp4 playing on the screen
#   2. crossfade 3D -> full-screen UI edit -> (fadeblack) stat card
#   3. mux a silent stereo track for platform compatibility
...
BOOT=5.6         # film.html T_BOOT: screen turns on, UI video t=0
XFADE_AT=8.2     # 3D time where we blend to the full-screen UI
XFADE_DUR=0.8
...
ffmpeg -y -v error \
  ...
  -i out/statcard.mp4 \
  -f lavfi -t 120 -i anullsrc=r=48000:cl=stereo \
  -filter_complex "\
    [0:v]settb=AVTB[base]; \
