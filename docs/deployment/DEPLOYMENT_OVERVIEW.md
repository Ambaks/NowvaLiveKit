...
- **[config/gunicorn_config_gcp.py](config/gunicorn_config_gcp.py)** - GCP-optimized Gunicorn config
...
### Existing Configs
- [config/gunicorn_config.py](config/gunicorn_config.py) - Mac configuration (4 workers)
- [config/nginx_production.conf](config/nginx_production.conf) - Mac Nginx config
...
1. **On Local Machine:**
   ...
   tar -czf nowva-deploy.tar.gz \
     src/ frontend_demo/dist/ config/gunicorn_config_gcp.py \
