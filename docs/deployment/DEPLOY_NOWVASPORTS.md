...
sudo cp config/nginx_production.conf /opt/homebrew/etc/nginx/servers/nowva.conf
...
```bash
# Edit nginx config
nano config/nginx_production.conf
```
...
nohup gunicorn -c config/gunicorn_config.py src.api.main:app > logs/gunicorn_startup.log 2>&1 &
```
...
# Restart services
brew services restart nginx
pkill -f gunicorn && nohup gunicorn -c config/gunicorn_config.py src.api.main:app &
```
