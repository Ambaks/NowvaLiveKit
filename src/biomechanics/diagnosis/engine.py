...
class HypothesisEngine:
    ...
    def _score_causes(
        ...
        """Score each candidate cause across all detected symptoms.

        Returns dict keyed by cause_id with aggregate_score and metadata.
        """
        ...
        for cause_id, posteriors in cause_posteriors.items():
            # Noisy-OR: aggregate_score = 1 - prod(1 - p_i)
    ...
    def _build_hypotheses(
        ...
        for cause_id, info in filtered_causes.items():
            ...
            # Compute parameter delta for tier-1 causes
    ...
    def _merge_perturbations(
        ...
        """Merge all tier-1 parameter deltas into a combined perturbation.

        Regular keys are summed (additive corrections).
        __foot_target_delta arrays are element-wise summed.
        """
    ...
    def _extract_feature(
        ...
        """Extract a raw or derived feature from a rep."""
        if feature_name == "knee_valgus_max":
            ...
            # Positive if hip is above knee (can't reach parallel)
    ...
    def _pick_representative_rep(
        ...
        """Pick the median rep by trunk pitch as the representative."""
        if len(reps) == 1:
            return reps[0]
    ...
    def _fill_template(
        ...
        """Fill explanation template with available values."""
        fill_values = {
