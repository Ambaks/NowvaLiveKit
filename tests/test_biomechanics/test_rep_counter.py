...
class TestRepCounterBasics:
    ...
    def test_custom_config(self):
        ...
        assert counter.config.entry_threshold == 40.0
...
class TestRepDetection:
    ...
    def test_rep_entry(self):
        """Counter should transition to IN_REP when threshold crossed."""
        counter = RepCounter()

        # Start below threshold
        angles = self.create_angles(0, hip_flexion=20.0, knee_flexion=20.0)
        result = counter.update(angles)
        assert counter.state == RepState.IDLE
        assert result is None

        # Cross threshold
        angles = self.create_angles(1, hip_flexion=35.0, knee_flexion=40.0)
        result = counter.update(angles)
        assert counter.state == RepState.IN_REP
    ...
    def test_rep_exit_minimum_duration(self):
        ...
        config = RepCounterConfig(min_rep_duration_frames=20)
        counter = RepCounter(config)

        # Enter rep
        angles = self.create_angles(0, hip_flexion=35.0, knee_flexion=40.0)
        counter.update(angles)
        assert counter.state == RepState.IN_REP

        # Exit immediately (duration too short)
        angles = self.create_angles(5, hip_flexion=15.0, knee_flexion=15.0)
        result = counter.update(angles)
        assert counter.state == RepState.IDLE
        assert counter.rep_count == 0
    ...
    def test_full_rep_cycle(self):
        ...
        counter.update(self.create_angles(1, hip_flexion=35.0, knee_flexion=40.0, timestamp=0.033))
        assert counter.state == RepState.IN_REP

        # Continue descending to max depth
        for i in range(2, 8):
            depth = 40.0 + (i - 2) * 10  # 40 -> 100 degrees
            counter.update(self.create_angles(i, hip_flexion=depth * 0.9, knee_flexion=depth, timestamp=i * 0.033))

        # Start ascending
        for i in range(8, 12):
            depth = 100.0 - (i - 8) * 20  # 100 -> 20 degrees
            counter.update(self.create_angles(i, hip_flexion=depth * 0.9, knee_flexion=depth, timestamp=i * 0.033))

        # Exit rep
        result = counter.update(self.create_angles(12, hip_flexion=10.0, knee_flexion=10.0, timestamp=0.4))
    ...
    def test_three_reps_sequence(self):
        ...
        for rep_idx in range(3):
            ...
            for step in range(15):
                ...
                result = counter.update(self.create_angles(frame, hip_flexion=hip, knee_flexion=knee, timestamp=frame * 0.033))
                frame += 1
    ...
    def test_depth_tracking(self):
        ...
        config = RepCounterConfig(min_rep_duration_frames=5)
        counter = RepCounter(config)

        # Enter rep
        counter.update(self.create_angles(0, hip_flexion=35.0, knee_flexion=40.0))
        ...
        counter.update(self.create_angles(5, hip_flexion=30.0, knee_flexion=35.0))

        # Complete rep
        result = counter.update(self.create_angles(6, hip_flexion=10.0, knee_flexion=15.0))
...
class TestRepTiming:
    ...
    def create_angles(self, frame: int, hip_flexion: float, timestamp: float) -> JointAngles:
        ...
        return JointAngles(
            hip_flexion_l=hip_flexion,
            hip_flexion_r=hip_flexion,
            knee_flexion_l=hip_flexion * 1.1,
            knee_flexion_r=hip_flexion * 1.1,
            frame_index=frame,
            timestamp=timestamp,
        )
    ...
    def test_rep_duration(self):
        ...
        counter.update(self.create_angles(0, hip_flexion=35.0, timestamp=0.0))
        counter.update(self.create_angles(1, hip_flexion=60.0, timestamp=0.5))
        counter.update(self.create_angles(2, hip_flexion=80.0, timestamp=1.0))  # Bottom
        counter.update(self.create_angles(3, hip_flexion=60.0, timestamp=1.3))
        counter.update(self.create_angles(4, hip_flexion=35.0, timestamp=1.6))
        result = counter.update(self.create_angles(5, hip_flexion=10.0, timestamp=2.0))
    ...
    def test_descent_ascent_timing(self):
        ...
        counter.update(self.create_angles(0, hip_flexion=35.0, timestamp=0.0))
        counter.update(self.create_angles(1, hip_flexion=60.0, timestamp=0.4))
        counter.update(self.create_angles(2, hip_flexion=90.0, timestamp=1.0))  # Bottom
        counter.update(self.create_angles(3, hip_flexion=60.0, timestamp=1.4))
        counter.update(self.create_angles(4, hip_flexion=35.0, timestamp=1.7))
        result = counter.update(self.create_angles(5, hip_flexion=10.0, timestamp=2.0))
...
class TestFaultIntegration:
    ...
    def test_add_fault_during_rep(self):
        ...
        # Start rep
        counter.update(self.create_angles(0, hip_flexion=35.0))
        ...
        counter.update(self.create_angles(1, hip_flexion=60.0))
        counter.update(self.create_angles(2, hip_flexion=80.0))
        counter.update(self.create_angles(3, hip_flexion=60.0))
        counter.update(self.create_angles(4, hip_flexion=35.0))
        result = counter.update(self.create_angles(5, hip_flexion=10.0))
    ...
    def test_faults_passed_to_update(self):
        ...
        counter.update(self.create_angles(0, hip_flexion=35.0), faults=[fault1])
        counter.update(self.create_angles(1, hip_flexion=60.0))
        counter.update(self.create_angles(2, hip_flexion=80.0), faults=[fault2])
        counter.update(self.create_angles(3, hip_flexion=60.0))
        counter.update(self.create_angles(4, hip_flexion=35.0))
        result = counter.update(self.create_angles(5, hip_flexion=10.0))
...
class TestAsymmetryTracking:
    ...
    def test_asymmetry_averaging(self):
        ...
        counter.update(self.create_asymmetric_angles(0, 35, 35, 45, 35))  # 10° diff
        counter.update(self.create_asymmetric_angles(1, 50, 50, 65, 55))  # 10° diff
        counter.update(self.create_asymmetric_angles(2, 70, 70, 85, 75))  # 10° diff
        counter.update(self.create_asymmetric_angles(3, 50, 50, 65, 55))
        counter.update(self.create_asymmetric_angles(4, 35, 35, 45, 35))
        result = counter.update(self.create_asymmetric_angles(5, 10, 10, 15, 15))
